...
class PrincipiaGame:
    ...
    def setup_ui(self):
        ...
        setup_btn = ttk.Button(control_buttons, text="Nowa Gra", command=self.show_game_config_dialog)
    ...
    def show_game_config_dialog(self):
        ...
        ttk.Radiobutton(mode_frame, text="Hostuj grę sieciową", variable=self.game_mode, value="host").pack(anchor='w', padx=5, pady=2)
        ttk.Radiobutton(mode_frame, text="Dołącz do gry", variable=self.game_mode, value="join").pack(anchor='w', padx=5, pady=2)
        ...
        # Konfiguracja sieciowa (dla trybu dołączania)
        network_frame = ttk.LabelFrame(config_window, text="Połączenie sieciowe")
    ...
    def update_player_entries(self):
        ...
        for i in range(self.player_count.get()):
            ...
            # Kolor gracza
            color_label = tk.Label(row_frame, text=f"●", fg=colors[i], font=('Arial', 16))
    ...
    def start_configured_game(self, config_window):
        """Rozpoczyna grę z podaną konfiguracją"""
    ...
    def setup_host_game(self, player_count, player_names):
        ...
        if self.game_server.start(self):
            ...
            self.log_message(f"🎮 Serwer uruchomiony na porcie {port}")
            self.log_message(f"🔗 Twoje IP: {local_ip}")
            self.log_message(f"📋 Gracze mogą się łączyć używając: {local_ip}:{port}")

            # Pokaż dialog z informacjami o serwerze
            self.show_server_info_dialog(local_ip, port)

            self.setup_game(player_count, player_names)
        else:
            messagebox.showerror("Błąd", "Nie udało się uruchomić serwera gry")
            self.is_network_game = False
            self.is_host = False
    ...
    def get_local_ip(self):
        ...
        try:
            # Połącz się z zewnętrznym serwerem aby znaleźć lokalne IP
    ...
    def show_server_info_dialog(self, ip, port):
        ...
        # Tytuł
        tk.Label(info_dialog, text="🎮 Serwer gry uruchomiony!",
        ...
        tk.Label(info_frame, text="Gracze mogą się łączyć używając:",
        ...
        def copy_to_clipboard():
            ...
            messagebox.showinfo("Skopiowano", f"Adres {connection_text} został skopiowany do schowka!")

        tk.Button(info_frame, text="📋 Skopiuj adres", command=copy_to_clipboard,
        ...
        instructions.insert(tk.END,
                          f"1. Przekaż ten adres innym graczom: {connection_text}\n\n"
                          "2. Gracze powinni wybrać 'Dołącz do gry' w menu\n\n"
                          "3. Wpisać powyższy adres w polu 'Adres hosta'\n\n"
                          "4. Gdy wszyscy się połączą, możesz rozpocząć grę")
    ...
    def join_network_game(self):
        ...
        tk.Label(name_dialog, text="Podaj swoją nazwę:", font=('Arial', 12)).pack(pady=10)
        ...
        tk.Button(button_frame, text="Połącz", command=confirm_name,
        ...
        if self.game_client.connect(host, port, player_name, self):
            self.log_message(f"🔗 Połączono z grą na {host}:{port} jako {player_name}")
            self.log_message("⏳ Oczekiwanie na rozpoczęcie gry...")

            # Ukryj normalny interfejs gry do czasu synchronizacji
            self.show_network_waiting_ui()
        else:
            messagebox.showerror("Błąd", f"Nie udało się połączyć z serwerem {host}:{port}")
            self.is_network_game = False

    def show_network_waiting_ui(self):
        """Pokazuje interfejs oczekiwania na grę sieciową"""
        # Wyczyść główny interfejs
        for tab_id in self.notebook.tabs():
            self.notebook.forget(tab_id)

        # Dodaj zakładkę oczekiwania
        waiting_frame = ttk.Frame(self.notebook)
        self.notebook.add(waiting_frame, text="🌐 Gra sieciowa")

        # Komunikat oczekiwania
        tk.Label(waiting_frame, text="🔗 Połączono z grą sieciową",
                font=('Arial', 16, 'bold'), fg='green').pack(pady=20)
        tk.Label(waiting_frame, text="⏳ Oczekiwanie na rozpoczęcie gry przez hosta...",
                font=('Arial', 12)).pack(pady=10)

        # Informacje o połączeniu
        info_frame = ttk.LabelFrame(waiting_frame, text="Informacje o połączeniu")
        ...
        tk.Button(waiting_frame, text="Rozłącz się", command=self.disconnect_from_network,
                 bg='red', fg='white', font=('Arial', 12)).pack(pady=20)

    def disconnect_from_network(self):
        """Rozłącza się z gry sieciowej"""
        if self.game_client:
            self.game_client.disconnect()
            self.game_client = None

        self.is_network_game = False
        self.log_message("👋 Rozłączono z gry sieciowej")

        # Przywróć normalny interfejs
    ...
    def setup_game(self, player_count=3, player_names=None):
        ...
        try:
            ...
            # Stwórz graczy z podaną konfiguracją
            colors = ['red', 'blue', 'green', 'purple']
    ...
    def send_action_to_network(self, action_type, action_data):
        ...
        if self.is_network_game and not self.is_host:
            ...
            if self.game_server:
                # Host rozgłasza akcję do wszystkich klientów
    ...
    def handle_network_action(self, message):
        """Obsługuje akcję od innego gracza przez sieć"""
        action_type = message.data.get('action_type')
        action_data = message.data.get('action_data')

        # TODO: Zaimplementować obsługę różnych typów akcji
    ...
    def handle_network_play_card(self, action_data):
        """Obsługuje zagranie karty przez innego gracza"""
        # TODO: Implementować
        pass

    def handle_network_hex_placement(self, action_data):
        """Obsługuje umieszczenie heksa przez innego gracza"""
        # TODO: Implementować
        pass

    def handle_network_research_start(self, action_data):
        """Obsługuje rozpoczęcie badania przez innego gracza"""
        # TODO: Implementować
        pass

    def broadcast_game_state(self):
        """Rozgłasza aktualny stan gry (tylko host)"""
        if self.is_network_game and self.is_host and self.game_server:
            # Zostanie wywołane przez serwer automatycznie
            pass

    def cleanup_network(self):
        """Czyści połączenia sieciowe"""
        if self.game_server:
    ...
    def run(self):
        ...
        try:
            ...
            # Wyczyść połączenia sieciowe przy zamykaniu
